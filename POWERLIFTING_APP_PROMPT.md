...
- 用户可以选择训练日期（不能晚于今天）。
- 用户可以选择训练日期（不能晚于今天）。日期区域采用图标 + 标签 + 日期值 + 箭头的横向布局，日历图标使用 `/assets/icon/date.png`。
...
- 选中动作后，页面以卡片形式展示当前动作：左侧图片缩略图（暂用首字兜底）、动作名称、1RM 标签及当前值、右上角"选择动作 >"按钮（打开动作选择弹窗）。
- 动作卡片下方平铺两个信息卡片：
  - "1RM 重量"卡片：显示当前 1RM（kg），提供"修改"按钮弹出 `wx.showModal({ editable: true })` 进行修改或清除。
  - "动作详情"卡片：标注"目标肌群 · 器械 · 要点"，点击打开动作详情弹窗。
- 每个动作可以添加多组训练数据。每组训练数据包含：
  - 重量（kg，允许 0，表示自重 / 空杆）。
  - 次数（>0）。
  - RPE（1-10）。
- 训练组区域顶部显示"重量 / 次数 / RPE"列头，每行平铺：序号圆形标记、重量输入 + 单位切换、次数输入、RPE 输入、删除按钮（使用 `/assets/icon/delete.png` 图标）。添加组数按钮仅在所有组的下方，以虚线框 `+ 添加组数` 呈现。
- 保存按钮带 `/assets/icon/save.png` 图标。
...
- UI 组件库使用 **Vant Weapp** (`@vant/weapp`)，已在 `app.json` 全局注册 `van-icon`、`van-popup`、`van-tag`、`van-overlay`、`van-transition`。使用前需在微信开发者工具中执行"构建 npm"。
- 不引入不必要的依赖。
- 功能改动应尽量集中且可验证。
